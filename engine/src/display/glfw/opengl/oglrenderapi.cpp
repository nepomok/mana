...
namespace mana {
    namespace opengl {
         ...
         */
        void presentScene(const RenderScene &scene, bool clearColor, bool clearDepth, bool clearStencil) {
            //Check scene
            ...
            view = MatrixMath::rotate(
                    Vec3f(scene.camera.transform.rotation.x,
                          scene.camera.transform.rotation.y,
                          scene.camera.transform.rotation.z));
            view = view * MatrixMath::translate(
                    Vec3f(-scene.camera.transform.position.x,
                          -scene.camera.transform.position.y,
                          -scene.camera.transform.position.z));

            //Calculate projection matrix
            if (scene.camera.cameraType == RenderCamera::PERSPECTIVE) {
                RenderCamera::PerspectiveData data = scene.camera.cameraData.perspective;
                projection = MatrixMath::perspective(glm::radians(data.fov), data.aspectRatio,
                                                     data.nearClip, data.farClip);
            } else {
                RenderCamera::OrthographicData data = scene.camera.cameraData.orthographic;
            ...
            for (auto &renderUnit : scene.units) {
        ...
        void OGLRenderAPI::render(const RenderScene &scene,
            ...
            presentScene(scene, clearColor, clearDepth, clearStencil);
        ...
        FrameBufferObject *OGLRenderAPI::allocateFrameBuffer(int width,
            ...
            if (wnd == NULL) {
                throw std::runtime_error("Attempted to allocateTexture framebuffer without a bound context.");
            }

            auto *ret = new OGLUserFrameBufferObject(wnd, width, height);
        ...
        FrameBufferObject *OGLRenderAPI::allocateFrameBuffer(const TextureObject &texture) {
            ...
            auto *ret = new OGLUserFrameBufferObject(wnd, tex.width, tex.height);
        ...
        TextureObject *OGLRenderAPI::allocateTexture(int width,
            ...
            GLFWwindow *wnd = glfwGetCurrentContext();
            if (wnd == NULL) {
                throw std::runtime_error("Attempted to allocateTexture texture without a bound context.");
            }

            auto *ret = new OGLTextureObject(wnd, width, height);
        ...
        TextureObject *OGLRenderAPI::allocateTexture(const ImageBuffer<ColorRGB24> &imageBuffer,
            ...
            auto *ret = new OGLTextureObject(wnd, imageBuffer.getWidth(), imageBuffer.getHeight());
        ...
        TextureObject *OGLRenderAPI::allocateTexture(const std::vector<ImageBuffer<ColorRGB24>> &imageBuffers,
            ...
            auto *ret = new OGLTextureObject(wnd, imageBuffers.at(0).getWidth(), imageBuffers.at(0).getHeight());
        ...
        MeshObject *OGLRenderAPI::allocateMesh(const RenderMesh &mesh) {
            GLFWwindow *wnd = glfwGetCurrentContext();
            if (wnd == NULL) {
                throw std::runtime_error("Attempted to allocateTexture meshData without a bound context.");
            }
            ...
            auto *ret = new OGLMeshObject(wnd);
        ...
        MeshObject *OGLRenderAPI::allocateMeshInstanced(const RenderMesh &mesh,
                                                        std::vector<RenderTransform> offsets) {
            GLFWwindow *wnd = glfwGetCurrentContext();
            if (wnd == NULL) {
                throw std::runtime_error("Attempted to allocateTexture meshData without a bound context.");
            }
