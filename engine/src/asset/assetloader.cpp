...
namespace mana {
            ...
            if (taskIterator == bundleTasks.end()) {
                bundleTasks[bundlePath] = pool.addTask(
                        [&archive, &bundleMutex, &refBundles, &bundlePath, &pool]() {
                            ...
                            auto bundle = AssetLoader::loadBundle(*stream, path.extension(), archive, pool);
    ...
    AssetBundle AssetLoader::loadBundle(std::istream &source,
        ...
        if (hint.empty()) {
            ...
            try {
                return readJsonBundle(source, archive, pool);
